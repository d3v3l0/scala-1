 ...
 */
trait GenSeqLike[+A, +Repr] extends Any with GenIterableLike[A, Repr] with Equals with Parallelizable[A, parallel.ParSeq[A]] { self =>
   ...
   */
  def segmentLength(@plocal p: A => Boolean, from: Int)(implicit @local mct: MaybeCanThrow): Int
   ...
   *           or `-1`, if none exists.
   */
  def indexWhere(p: A => Boolean, from: Int)(implicit @local mct: MaybeCanThrow): Int
   ...
   *           or `-1`, if none exists.
   */
  def indexWhere(p: A => Boolean)(implicit @local mct: MaybeCanThrow): Int = indexWhere(p, 0)
   ...
   *           or `-1`, if none exists.
   */
  def lastIndexWhere(p: A => Boolean)(implicit @local mct: MaybeCanThrow): Int = lastIndexWhere(p, length - 1)
   ...
   *           or `-1`, if none exists.
   */
  def lastIndexWhere(p: A => Boolean, end: Int)(implicit @local mct: MaybeCanThrow): Int
   ...
   *
   *  @usecase def reverseMap[B](f: A => B, @local mct: MaybeCanThrow): $Coll[B]
   *    @inheritdoc
   *
   *    $willNotTerminateInf
   *
   ...
   *                  `f` to each element of this $coll and collecting the results in reversed order.
   */
  def reverseMap[B, That](f: A => B)(implicit bf: CanBuildFrom[Repr, B, That], @local mct: MaybeCanThrow): That
   ...
   */
  def startsWith[B](that: GenSeq[B])(implicit @local mct: MaybeCanThrow): Boolean = startsWith(that, 0)
   ...
   */
  def startsWith[B](that: GenSeq[B], offset: Int)(implicit @local mct: MaybeCanThrow): Boolean
   ...
   */
  def endsWith[B](that: GenSeq[B])(implicit @local mct: MaybeCanThrow): Boolean
   ...
   *                   by `patch`.
   *
   *  @usecase def patch(from: Int, that: GenSeq[A], replaced: Int, @local mct: MaybeCanThrow): $Coll[A]
   *    @inheritdoc
   *
   *    @return          a new $coll consisting of all elements of this $coll
   *                     except that `replaced` elements starting from `from` are replaced
   *                     by `patch`.
   */
  def patch[B >: A, That](from: Int, patch: GenSeq[B], replaced: Int)(implicit bf: CanBuildFrom[Repr, B, That], @local mct: MaybeCanThrow): That
   ...
   */
  def updated[B >: A, That](index: Int, elem: B)(implicit bf: CanBuildFrom[Repr, B, That], @local mct: MaybeCanThrow): That
   ...
   *  @usecase def padTo(len: Int, elem: A, @local mct: MaybeCanThrow): $Coll[A]
   *    @inheritdoc
   *
   *    @return a new $coll consisting of
   *            all elements of this $coll followed by the minimal number of occurrences of `elem` so
   ...
   */
  def padTo[B >: A, That](len: Int, elem: B)(implicit bf: CanBuildFrom[Repr, B, That], @local mct: MaybeCanThrow): That
   ...
   *                  followed by all elements of `that`.
   */
  def union[B >: A, That](that: GenSeq[B])(implicit bf: CanBuildFrom[Repr, B, That], @local mct: MaybeCanThrow): That = this ++ that
   ...
   */
  override def equals(that: Any): Boolean = { @local implicit val mcc = new CanThrow {}; that match { // XXX(leo)
