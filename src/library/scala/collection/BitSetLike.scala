...
trait BitSetLike[+This <: BitSetLike[This] with SortedSet[Int]] extends SortedSetLike[Int, This] { self =>
  ...
  override def isEmpty(implicit @local mct: MaybeCanThrow = mct): Boolean = 0 until nwords forall (i => word(i) == 0)
  ...
  override def foreach[B](f: Int => B)(implicit @local mct: MaybeCanThrow = mct) {
