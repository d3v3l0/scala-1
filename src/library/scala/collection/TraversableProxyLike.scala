...
trait TraversableProxyLike[+A, +Repr <: TraversableLike[A, Repr] with Traversable[A]] extends TraversableLike[A, Repr] with Proxy {
  ...
  override def filter(@local p: A => Boolean): Repr = self.filter(p)
  override def filterNot(@local p: A => Boolean): Repr = self.filterNot(p)
  override def collect[B, That](pf: PartialFunction[A, B])(implicit bf: CanBuildFrom[Repr, B, That]): That = self.collect(pf)(bf)
  override def partition(@local p: A => Boolean): (Repr, Repr) = self.partition(p)
  override def groupBy[K](f: A => K): immutable.Map[K, Repr] = self.groupBy(f)
  override def forall(@local p: A => Boolean): Boolean = self.forall(p)
  override def exists(@local p: A => Boolean): Boolean = self.exists(p)
  override def count(@local p: A => Boolean): Int = self.count(p)
  override def find(@local p: A => Boolean): Option[A] = self.find(p)
  override def foldLeft[B](z: B)(@local op: (B, A) => B): B = self.foldLeft(z)(op)
  ...
  override def reduceLeft[B >: A](@local op: (B, A) => B): B = self.reduceLeft(op)
  ...
  override def takeWhile(@local p: A => Boolean): Repr = self.takeWhile(p)
  override def dropWhile(@local p: A => Boolean): Repr = self.dropWhile(p)
  override def span(@local p: A => Boolean): (Repr, Repr) = self.span(p)
