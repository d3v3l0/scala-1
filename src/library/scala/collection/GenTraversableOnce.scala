...
trait GenTraversableOnce[+A] extends Any {
   ...
   */
  def reduce[A1 >: A](@plocal op: (A1, A1) => A1)(implicit @local mct: MaybeCanThrow): A1
   ...
   */
  def reduceOption[A1 >: A](@plocal op: (A1, A1) => A1)(implicit @local mct: MaybeCanThrow): Option[A1]
   ...
   */
  def fold[A1 >: A](z: A1)(@plocal op: (A1, A1) => A1)(implicit @local mct: MaybeCanThrow): A1
   ...
   */
  def aggregate[B](z: =>B)(@plocal seqop: (B, A) => B, @plocal combop: (B, B) => B)(implicit @local mct: MaybeCanThrow): B
   ...
   */
  def count(p: A => Boolean)(implicit @local mct: MaybeCanThrow): Int
   ...
   *
   */
  def sum[A1 >: A](implicit num: Numeric[A1], @local mct: MaybeCanThrow): A1
   ...
   *     Examples of such types are: `Long`, `Float`, `Double`, `BigInt`.
   */
  def product[A1 >: A](implicit num: Numeric[A1], @local mct: MaybeCanThrow): A1
   ...
   */
  def min[A1 >: A](implicit ord: Ordering[A1], @local mct: MaybeCanThrow): A
   ...
   */
  def max[A1 >: A](implicit ord: Ordering[A1], @local mct: MaybeCanThrow): A
   ...
   *
   *  @usecase def maxBy[B](f: A => B, @local mct: MaybeCanThrow): A
   *    @inheritdoc
   *
   *    @return   the first element of this $coll with the largest value measured by function f.
   */
  def maxBy[B](@plocal f: A => B)(implicit cmp: Ordering[B], @local mct: MaybeCanThrow): A
   ...
   *  with respect to the ordering `cmp`.
   *
   *  @usecase def minBy[B](f: A => B, @local mct: MaybeCanThrow): A
   *    @inheritdoc
   *
   *    @return   the first element of this $coll with the smallest value measured by function f.
   */
  def minBy[B](@plocal f: A => B)(implicit cmp: Ordering[B], @local mct: MaybeCanThrow): A

  def forall(@plocal pred: A => Boolean)(implicit @local mct: MaybeCanThrow): Boolean

  def exists(pred: A => Boolean)(implicit @local mct: MaybeCanThrow): Boolean
   ...
   */
  def find(@plocal pred: A => Boolean)(implicit @local mct: MaybeCanThrow): Option[A]
   ...
   *
   *  @usecase def copyToArray(xs: Array[A])(implicit @local mct: MaybeCanThrow): Unit
   *    @inheritdoc
   *
   *    $willNotTerminateInf
   */
   ...
   *  @tparam B      the type of the elements of the array.
   *
   *  @usecase def copyToArray(xs: Array[A], start: Int)(implicit @local mct: MaybeCanThrow): Unit
   *    @inheritdoc
   *
   *    $willNotTerminateInf
   */
  ...
  def copyToArray[B >: A](xs: Array[B], start: Int, len: Int)(implicit @local mct: MaybeCanThrow): Unit
   ...
   */
  def toMap[K, V](implicit ev: A <:< (K, V), @local mct: MaybeCanThrow): GenMap[K, V]
   ...
   *    @return a new collection containing all elements of this $coll.
   */
  def to[Col[_]](implicit cbf: CanBuildFrom[Nothing, A, Col[A @uV]], @local mct: MaybeCanThrow): Col[A @uV]
}
