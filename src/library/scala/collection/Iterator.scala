...
object Iterator {
  ...
  val empty: Iterator[Nothing] = new AbstractIterator[Nothing] {
    ...
    @local def next(): Nothing = throw new NoSuchElementException("next on empty iterator")
  }
  ...
  def from(start: Int, step: Int): Iterator[Int] = new AbstractIterator[Int] {
    private var i = start
    def hasNext: Boolean = true
    @local def next(): Int = { val result = i; i += step; result }
  }
...
trait Iterator[+A] extends TraversableOnce[A] {
   ...
   *  @note   Reuse: $consumesAndProducesIterator
   */
  def map[B](f: A => B)(implicit @local mct: MaybeCanThrow = mct): Iterator[B] = new AbstractIterator[B] { // TODO(leo) parametrize with LT
  ...
  def flatMap[B](f: A => GenTraversableOnce[B], @local mct: MaybeCanThrow = mct): Iterator[B] = new AbstractIterator[B] {
    ...
    @local def next(): B = (if (hasNext) cur else empty).next()
  }
   ...
   *  @note    Reuse: $consumesAndProducesIterator
   */
  def filter(p: A => Boolean)(implicit @local mct: MaybeCanThrow = mct): Iterator[A] = new AbstractIterator[A] {
    private var hd: A = _
    ...
    @local def next() = if (hasNext) { hdDefined = false; hd } else empty.next()
  }
   ...
   *  @note    Reuse: $consumesAndProducesIterator
   */
  def withFilter(p: A => Boolean)(implicit @local mct: MaybeCanThrow = mct): Iterator[A] = filter(p)
  ...
  */
  ...
  def collect[B](pf: PartialFunction[A, B], @local mct: MaybeCanThrow = mct): Iterator[B] = {
    ...
    new AbstractIterator[B] {
      ...
      @local def next() = { skip(); pf(self.next()) }
    }
  }
  ...
  def partition(p: A => Boolean)(implicit @local mct: MaybeCanThrow = mct): (Iterator[A], Iterator[A]) = {
    ...
    class PartitionIterator(p: A => Boolean) extends AbstractIterator[A] {
      ...
      @local def next() = if (!lookahead.isEmpty) lookahead.dequeue()
                   else { skip(); self.next() }
    }
  ...
  def duplicate: (Iterator[A], Iterator[A]) = {
    ...
    class Partner extends AbstractIterator[A] {
      ...
      @local def next(): A = self.synchronized {
