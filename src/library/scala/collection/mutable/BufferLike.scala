...
{ self : This =>
   ...
   *            `0 <= n <= length`.
   */
  def insertAll(n: Int, elems: scala.collection.Traversable[Any, A])
   ...
   *  @return the buffer itself.
   */
  def ++=:(xs: TraversableOnce[Any, A]): this.type = { insertAll(0, xs.toTraversable); this }
   ...
   */
  def appendAll(xs: TraversableOnce[Any, A]) { this ++= xs }
   ...
   */
  def prependAll(xs: TraversableOnce[Any, A]) { xs ++=: this }
  ...
  def readOnly: scala.collection.Seq[Any, A] = toSeq
  ...
  def ++(xs: GenTraversableOnce[Any, A]): This = clone() ++= xs.seq
