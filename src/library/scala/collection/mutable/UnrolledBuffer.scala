   ...
   with Serializable
{
  ...
  def insertAll(idx: Int, elems: scala.collection.Traversable[Any, T]) =
    if (idx >= 0 && idx <= sz) {
...
object UnrolledBuffer extends ClassTagTraversableFactory[UnrolledBuffer] {
  ...
  class Unrolled[T: ClassTag] private[collection] (var size: Int, var array: Array[T], var next: Unrolled[T], val buff: UnrolledBuffer[T] = null) {
    ...
    @tailrec final def insertAll(idx: Int, t: scala.collection.Traversable[Any, T], buffer: UnrolledBuffer[T]): Unit = {
      if (idx < size) {
