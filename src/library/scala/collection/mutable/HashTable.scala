...
trait HashTable[A, Entry >: Null <: HashEntry[A, Entry]] extends HashTable.HashUtils[A] {
   ...
   */
  private[collection] def init(in: java.io.ObjectInputStream, readEntry: => Entry)(@local cc: CanThrow) {
  ESC.THROW{
   ...
   */
  private[collection] def serializeTo(out: java.io.ObjectOutputStream, writeEntry: Entry => Unit)(@local cc: CanThrow) {
  ESC.THROW{
