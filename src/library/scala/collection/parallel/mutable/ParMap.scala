...
extends GenMap[L, K, V]
   ...
   with ParMapLike[L, K, V, ParMap[L, K, V], mutable.Map[L, K, V]]
{
  ...
  def seq: scala.collection.mutable.Map[L, K, V]
   ...
   */
  def withDefault(d: K => V): scala.collection.parallel.mutable.ParMap[L, K, V] = new ParMap.WithDefault[K, V](this, d)
   ...
   *  @return      a wrapper of the map with a default value
   */
  def withDefaultValue(d: V): scala.collection.parallel.mutable.ParMap[L, K, V] = new ParMap.WithDefault[K, V](this, x => d)
}

object ParMap extends ParMapFactory[L, ParMap] {
  def empty[K, V]: ParMap[L, K, V] = new ParHashMap[L, K, V]

  def newCombiner[K, V]: Combiner[L, (K, V), ParMap[L, K, V]] = ParHashMapCombiner.apply[K, V]
  ...
  extends scala.collection.parallel.ParMap.WithDefault(underlying, d) with ParMap[L, K, V] {
     ...
     */
    override def withDefault(d: K => V): ParMap[L, K, V] = new WithDefault[K, V](underlying, d)
    override def withDefaultValue(d: V): ParMap[L, K, V] = new WithDefault[K, V](underlying, x => d)
  }
}
