...
sealed abstract class List[+A] extends AbstractSeq[A]
  ...
  override def ++[B >: A, That](that: GenTraversableOnce[B])(implicit bf: CanBuildFrom[List[A], B, That], @local mct: MaybeCanThrow): That =
  ...
  @inline final override def foreach[U](@local f: A => U)(implicit @local mct: MaybeCanThrow = mct) {
    var these = this
    while (!these.isEmpty) {
      f(these.head)
      these = these.tail
...
case object Nil extends List[Nothing] {
  ...
  override def equals(that: Any) = that match {
    case that1: scala.collection.GenSeq[_] => ESC.TRY(cc => that1.isEmpty(cc)) // XXX leo
    case _ => false
  }
}
