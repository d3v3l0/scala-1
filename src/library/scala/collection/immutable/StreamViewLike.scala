...
{ self =>
  ...
  protected override def newZipped[B](that: scala.collection.GenIterable[L, B]): Transformed[(A, B)] = new { val other = that } with AbstractTransformed[(A, B)] with Zipped[B]
  protected override def newZippedAll[A1 >: A, B](that: scala.collection.GenIterable[L, B], _thisElem: A1, _thatElem: B): Transformed[(A1, B)] = {
  ...
  protected override def newPatched[B >: A](_from: Int, _patch: scala.collection.GenSeq[L, B], _replaced: Int): Transformed[B] = {
