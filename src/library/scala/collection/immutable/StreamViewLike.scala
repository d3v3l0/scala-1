...
{ self =>
  ...
  /** boilerplate */
  protected override def newForced[B](xs: => scala.collection.GenSeq[Any, B]): Transformed[B] = new { val forced = xs } with AbstractTransformed[B] with Forced[B]
  protected override def newAppended[B >: A](that: scala.collection.GenTraversable[Any, B]): Transformed[B] = new { val rest = that } with AbstractTransformed[B] with Appended[B]
  protected override def newMapped[B](f: A => B): Transformed[B] = new { val mapping = f } with AbstractTransformed[B] with Mapped[B]
  protected override def newFlatMapped[B](f: A => scala.collection.GenTraversableOnce[Any, B]): Transformed[B] = new { val mapping = f } with AbstractTransformed[B] with FlatMapped[B]
  ...
  protected override def newZipped[B](that: scala.collection.GenIterable[Any, B]): Transformed[(A, B)] = new { val other = that } with AbstractTransformed[(A, B)] with Zipped[B]
  protected override def newZippedAll[A1 >: A, B](that: scala.collection.GenIterable[Any, B], _thisElem: A1, _thatElem: B): Transformed[(A1, B)] = {
  ...
  protected override def newPatched[B >: A](_from: Int, _patch: scala.collection.GenSeq[Any, B], _replaced: Int): Transformed[B] = {
