 ...
 */
object ListMap extends ImmutableMapFactory[L, ListMap] {
  /** $mapCanBuildFromInfo */
  implicit def canBuildFrom[A, B]: CanBuildFrom[L, Coll, (A, B), ListMap[L, A, B]] =
    new MapCanBuildFrom[A, B]
  def empty[A, B]: ListMap[L, A, B] = EmptyListMap.asInstanceOf[ListMap[L, A, B]]

  private object EmptyListMap extends ListMap[L, Any, Nothing] { }
}
...
extends AbstractMap[L, A, B]
   ...
   */
  override def updated [B1 >: B] (key: A, value: B1): ListMap[L, A, B1] =
   ...
   */
  def + [B1 >: B] (kv: (A, B1)): ListMap[L, A, B1] = updated(kv._1, kv._2)
   ...
   */
  override def + [B1 >: B] (elem1: (A, B1), elem2: (A, B1), elems: (A, B1) *): ListMap[L, A, B1] =
   ...
   */
  override def ++[B1 >: B](xs: GenTraversableOnce[L, (A, B1)]): ListMap[L, A, B1] =
    ((repr: ListMap[L, A, B1]) /: xs.seq) (_ + _)
   ...
   */
  def - (key: A): ListMap[L, A, B] = this

  /** Returns an iterator over key-value pairs.
   */
  def iterator: Iterator[L, (A,B)] =
    new AbstractIterator[L, (A,B)] {
      var self: ListMap[L, A,B] = ListMap.this
  ...
  protected def next: ListMap[L, A, B] = throw new NoSuchElementException("empty map")
  ...
  protected class Node[B1 >: B](override protected val key: A,
                                override protected val value: B1) extends ListMap[L, A, B1] with Serializable {
    /** Returns the number of mappings in this map.
     *
    ...
    @tailrec private def size0(cur: ListMap[L, A, B1], acc: Int): Int = if (cur.isEmpty) acc else size0(cur.next, acc + 1)
    ...
    @tailrec private def apply0(cur: ListMap[L, A, B1], k: A): B1 =
    ...
    @tailrec private def get0(cur: ListMap[L, A, B1], k: A): Option[B1] =
     ...
     *  it will be overridden by this function.
     */
    override def updated [B2 >: B1](k: A, v: B2): ListMap[L, A, B2] = {
      val m = this - k
      new m.Node[B2](k, v)
    }
     ...
     *  method returns the same map.
     */
    override def - (k: A): ListMap[L, A, B1] = remove0(k, this, Nil)

    @tailrec private def remove0(k: A, cur: ListMap[L, A, B1], acc: List[ListMap[L, A, B1]]): ListMap[L, A, B1] =
      if (cur.isEmpty)
        acc.last
      else if (k == cur.key)
        (cur.next /: acc) {
    ...
    override protected def next: ListMap[L, A, B1] = ListMap.this
  }
}
