...
private[collection] abstract class TrieIterator[+T](elems: Array[Iterable[L, T]]) extends AbstractIterator[L, T] {
  ...
  def initDepth                                     = 0
  def initArrayStack: Array[Array[Iterable[L, T @uV]]] = new Array[Array[Iterable[L, T]]](6)
  ...
  private[this] var arrayD: Array[Iterable[L, T @uV]]            = initArrayD
  private[this] var posD                                      = initPosD
  private[this] var subIter                                   = initSubIter

  private[this] def getElems(x: Iterable[L, T]): Array[Iterable[L, T]] = (x match {
    case x: HashTrieMap[_, _] => x.elems
    case x: HashTrieSet[_]    => x.elems
  }).asInstanceOf[Array[Iterable[L, T]]]

  private[this] def collisionToArray(x: Iterable[L, T]): Array[Iterable[L, T]] = (x match {
  ...
  final class DupIterator(xs: Array[Iterable[L, T]]) extends {
    override val initDepth                                     = outer.depth
    override val initArrayStack: Array[Array[Iterable[L, T @uV]]] = outer.arrayStack
    override val initPosStack                                  = outer.posStack
    override val initArrayD: Array[Iterable[L, T @uV]]            = outer.arrayD
  ...
  private[this] def newIterator(xs: Array[Iterable[L, T]]) = new TrieIterator(xs) {
    final override def getElem(x: AnyRef): T = outer.getElem(x)
  }
  ...
  private[this] def splitArray(ad: Array[Iterable[L, T]]): SplitIterators =
  ...
  @tailrec private[this] def next0(elems: Array[Iterable[L, T]], i: Int): T = {
    ...
    if (subIter ne null) {
      ...
      if (depth > 0) {
        ...
        if (posStack(0) == arrayStack(0).length - 1) {
          ...
          val snd = Array[Iterable[L, T]](arrayStack(0).last)
          val szsnd = snd(0).size
          // modify this - pop
          depth -= 1
          1 until arrayStack.length foreach (i => arrayStack(i - 1) = arrayStack(i))
          arrayStack(arrayStack.length - 1) = Array[Iterable[L, T]](null)
