...
object Set extends ImmutableSetFactory[Set] {
  ...
  class Set1[A] private[collection] (elem1: A) extends AbstractSet[A] with Set[A] with Serializable {
    ...
    override def exists(@plocal f: A => Boolean)(implicit @local mct: MaybeCanThrow = mct): Boolean = {
      f(elem1)
    }
    override def forall(@plocal f: A => Boolean)(implicit @local mct: MaybeCanThrow = mct): Boolean = {
      f(elem1)
    }
    override def find(@plocal f: A => Boolean)(implicit @local mct: MaybeCanThrow = mct): Option[A] = {
