...
class TreeSet[A] private (tree: RB.Tree[A, Unit])(implicit val ordering: Ordering[A])
   ...
   */
  def + (elem: A)(implicit @local mct: MaybeCanThrow = mct): TreeSet[A] = newSet(RB.update(tree, elem, (), overwrite = false))
   ...
   */
  def - (elem:A)(implicit @local mct: MaybeCanThrow = mct): TreeSet[A] =
  ...
  override def foreach[U](f: A =>  U)(implicit @local mct: MaybeCanThrow = mct) = RB.foreachKey(tree, f)
