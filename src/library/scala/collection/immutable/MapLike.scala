...
self =>
   ...
   */
  override def updated [B1 >: B](key: A, value: B1): immutable.Map[L, A, B1] = this + ((key, value))

  /** Add a key/value pair to this map, returning a new map.
   *  @param    kv the key/value pair.
   *  @return   A new map with the new binding added to this map.
   */
  def + [B1 >: B] (kv: (A, B1)): immutable.Map[L, A, B1]
   ...
   */
  override def + [B1 >: B] (elem1: (A, B1), elem2: (A, B1), elems: (A, B1) *): immutable.Map[L, A, B1] =
   ...
   */
  override def keySet: immutable.Set[L, A] = new ImmutableDefaultKeySet

  protected class ImmutableDefaultKeySet extends super.DefaultKeySet with immutable.Set[L, A] {
    override def + (elem: A): immutable.Set[L, A] =
      if (this(elem)) this
      else immutable.Set[L, A]() ++ this + elem
    override def - (elem: A): immutable.Set[L, A] =
      if (this(elem)) immutable.Set[L, A]() ++ this - elem
      else this
  }
