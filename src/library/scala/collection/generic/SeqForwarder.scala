...
trait SeqForwarder[+A] extends Seq[A] with IterableForwarder[A] {
  ...
  override def segmentLength(@local p: A => Boolean, from: Int): Int = underlying.segmentLength(p, from)
  override def prefixLength(@local p: A => Boolean) = underlying prefixLength p
