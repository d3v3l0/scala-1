...
trait IterableForwarder[+A] extends Iterable[A] with TraversableForwarder[A] { self =>

  /** The iterable object to which calls are forwarded */
  protected def underlying: Iterable[A] { type LT = self.LT }

  // Iterable delegates
  // Iterable methods could be printed by  cat IterableLike.scala | sed -n '/trait Iterable/,$ p' | egrep '^  (override )?def'

  override def iterator: Iterator[A] = underlying.iterator
  override def sameElements[B >: A](that: GenIterable[B])(implicit @local mct: MaybeCanThrow = mct): Boolean = underlying.sameElements(that)
}
