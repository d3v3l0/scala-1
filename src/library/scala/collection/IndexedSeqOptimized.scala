...
trait IndexedSeqOptimized[+A, +Repr] extends Any with IndexedSeqLike[A, Repr] { self =>
  ...
  override /*IterableLike*/
  def zip[A1 >: A, B, That](that: GenIterable[Any, B])(implicit bf: CanBuildFrom[Repr, (A1, B), That]): That = that match {
  ...
  override /*IterableLike*/
  def sameElements[B >: A](that: GenIterable[Any, B]): Boolean = that match {
    case that: IndexedSeq[_] =>
      val len = length
      len == that.length && {
        var i = 0
  ...
  override /*SeqLike*/
  def startsWith[B](that: GenSeq[Any, B], offset: Int): Boolean = that match {
    case that: IndexedSeq[_] =>
      var i = offset
      var j = 0
      val thisLen = length
  ...
  override /*SeqLike*/
  def endsWith[B](that: GenSeq[Any, B]): Boolean = that match {
    case that: IndexedSeq[_] =>
      var i = length - 1
      var j = that.length - 1
